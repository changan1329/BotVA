...
class DisLayer_GAT(nn.Module):
    def __init__(self, num_edge_type, in_size, out_size, layer_num_heads, semantic_head, dropout):
        super(DisLayer_GAT, self).__init__()
        self.gated = nn.Sequential(
            nn.Linear(in_size + out_size, in_size),
            nn.Sigmoid()
        )
    ...
    def forward(self,features,edge_index_list):
        u = self.gat_layers[0](features, edge_index_list[0].squeeze(0)).flatten(1)
        a = self.gated(torch.cat((u, features), dim = 1))
        semantic_embeddings = (torch.mul(torch.tanh(u), a) + torch.mul(features, (1-a))).unsqueeze(1)
        for i in range(1,len(edge_index_list)):
            u = self.gat_layers[i](features, edge_index_list[i].squeeze(0)).flatten(1)
...
class DisLayer_GCN(nn.Module):
    def __init__(self, num_edge_type, in_size, out_size, layer_num_heads, semantic_head, dropout):
        super(DisLayer_GCN, self).__init__()
        self.gated = nn.Sequential(
            nn.Linear(in_size + out_size, in_size),
            nn.Sigmoid()
        )
...
class DisLayer_SAGE(nn.Module):
    def __init__(self, num_edge_type, in_size, out_size, layer_num_heads, semantic_head, dropout):
        super(DisLayer_SAGE, self).__init__()
        self.gated = nn.Sequential(
            nn.Linear(in_size + out_size, in_size),
            nn.Sigmoid()
        )
...
class DisLayer_no_gate(nn.Module):
    def __init__(self, num_edge_type, in_size, out_size, layer_num_heads, semantic_head, dropout):
        super(DisLayer_no_gate, self).__init__()
        self.gated = nn.Sequential(
            nn.Linear(in_size + out_size, in_size),
            nn.Sigmoid()
        )
...
class DisLayer_no_trans(nn.Module):
    def __init__(self, num_edge_type, in_size, out_size, layer_num_heads, semantic_head, dropout):
        super(DisLayer_no_trans, self).__init__()
